...
const Sidebar=()=>{
    const theme = useTheme();
    const colors = tokens(theme.palette.mode);
    const [isCollapsed, setIsCollapsed] = useState(false);     //check whter the sidebar is closed or open
    const [selected, setSelected] = useState("Dashboard");     //represents which page is selected currently

  
  return (
      <Box
            sx={{    //this css will override the original sidebar css properties
      ...
      <ProSidebar collapsed={isCollapsed}>
        <Menu iconShape="square">
          {/* LOGO AND MENU ICON */}
              ...
              {/* user profile logo section in sidebar*/}
              {!isCollapsed && (      
                  <Box mb="25px">
                    <Box display="flex" justifyContent="center" alignItems="center">
                      ...
                      <Box textAlign="center">
                        <Typography variant="h2" color={colors.grey[100]} fontWeight="bold" sx={{ m: "10px 0 0 0" }}>Lisa De Snow </Typography>
                        <Typography variant="h5" color={colors.greenAccent[500]}> Snr Data Analyst </Typography>
                      </Box>
                  </Box>
              )}
